...
void ModuleManager::load(Messenger* messenger,
    ...
    for(auto& config : configs) {
		...
		if(config.getName() == "TargetGeometryBuilderGeant4"){
...
/**
 * @throws InvalidModuleStateException If the module fails to forward the target to the base class
 *
 * For target modules multiple instantiations may be created per section. An instantiation is created for every detector if
 * no selection parameters are provided. Otherwise instantiations are created for every linked detector name and type.
 */
/*std::vector<std::pair<ModuleIdentifier, Module*>> ModuleManager::create_target_modules(
    void* library, Configuration& config, Messenger* messenger, GeometryManager* geo_manager, std::mt19937_64& seeder) {
    std::string module_name = config.getName();
    LOG(DEBUG) << "Creating instantions for target module " << module_name;
    ...
    // Convert to correct generator function
    auto module_generator =
        reinterpret_cast<Module* (*)(Configuration&, Messenger*, std::shared_ptr<Builder>)>(generator); // NOLINT

    // Handle empty type and name arrays:
    bool instances_created = false;
    std::vector<std::pair<std::shared_ptr<Builder>, ModuleIdentifier>> instantiations;
    ...
    if(config.has("name")) {
        std::vector<std::string> names = config.getArray<std::string>("name");
        for(auto& name : names) {
            auto det = geo_manager->getBuilders(name);
            instantiations.emplace_back(det, ModuleIdentifier(module_name, det->getName() + identifier, 0));
    ...
    // Create for all targets if no name / type provided
    if(!instances_created) {
        auto targets = geo_manager->getBuilders();

        for(auto& tar : targets) {
            instantiations.emplace_back(tar, ModuleIdentifier(module_name, "tar->getName() + identifier, 2));
        }
    }
    ...
    std::vector<std::pair<ModuleIdentifier, Module*>> module_list;
    for(auto& instance : instantiations) {
        LOG(DEBUG) << "Creating target instantiation " << instance.second.getUniqueName();
        // Get current time
        auto start = std::chrono::steady_clock::now();
        ...
        // Check if the module called the correct base class constructor
        if(module->getBuilders().get() != instance.first.get()) {
            throw InvalidModuleStateException(
                "Module " + module_name +
                " does not call the correct base Module constructor: the provided target should be forwarded");
        }
